...
const UpdateForm = (props) => {
    ...
    useEffect(() => {
        ...
        async function fetchData() {
            ...
            const supplier = await response.data.data
...
export default function Supplier() {
    ...
    async function getSuppliers() {
        ...
        var suppliers = result.data.data
                                    ...
                                    {   
                                        suppliers.map((supplier, index) => {
                                            return (
                                                <tr key={supplier.id}>
                                                    <th>{ index + 1 }</th>
                                                    <td><span className='d-inline-block text-truncate' style={{minWidth: "200px", maxWidth: "20vw"}}>{ supplier.name }</span></td>
                                                    <td><span className='d-inline-block text-truncate' style={{maxWidth: "20vw"}}>{ supplier.address }</span></td>
